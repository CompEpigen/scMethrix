...
#' 
#' If assays stored in HDF5-format are used, transform functions need to accept [`HDF5Array::HDF5Array`] or [`DelayedArray`](https://bioconductor.org/packages/release/bioc/html/DelayedArray.html)-derived  matrices (e.g., [`DelayedMatrixStats`](https://bioconductor.org/packages/release/bioc/html/DelayedMatrixStats.html)).
...
#---- bin_scMethrix ----------------------------------------------------------------------------------------------------
#' Bins the ranges of an [`scMethrix-class`] object.
#' @details Uses the inputted function to transform an assay in the [`scMethrix-class`] object. Typically, most assays will use either `mean` (for measurements) or `sum` (for counts). The transform is applied column-wise to optimize how HDF5 files access sample data. If HDF5 objects are used, transform functions should be from [`DelayedMatrixStats`](https://bioconductor.org/packages/release/bioc/html/DelayedMatrixStats.html).
#' 
#' In the output object, the number of CpGs in each region is saved in `rowData(scm)$n_cpgs`.
#' 
...
#' @param bin_by `string`; can create bins by # of base pairs `bp` or by # of CpG sites `cpg.` Default = `bp`.
#' @param trans `named vector of functions`; The transforms for each assay in a named vector. Default `NULL`, meaning that operations for `counts` assay is `sum(x, na.rm=TRUE)`, and for all other assays is `mean(x, na.rm=TRUE)`
...
#' @param batch_size `integer`; The number of CpGs to calculate at once.
#' @return An [`scMethrix-class`] object
#' @examples
#' data('scMethrix_data')
...
#---- impute_by_melissa ------------------------------------------------------------------------------------------------
#' Imputes the NA values of a [`scMethrix-class`] object.
#' @details Uses the inputted function to transform an assay in the [`scMethrix-class`] object
#' @param threshold `integer`; The value for cutoff in the "score" assay to determine methylated or unmethylated status. 
#' Default = 50
#' @inheritParams generic_scMethrix_function
#' @return An [`scMethrix-class`] object
#' @examples
...
#' @param regions [`GRanges`][GenomicRanges::GRanges()]; the regions to impute. Default is by chromosome.
#' @param type `string` or `function`; the imputation to perform `kNN`,`iPCA`,or `RF`. Otherwise, a function can be specified that returns the imputed matrix. Default = `kNN`
#' @param n_pc `list(integer)`; the range of principal components to check when using `iPCA`. Caution: this can be very time-intensive
#' @inheritParams generic_scMethrix_function
