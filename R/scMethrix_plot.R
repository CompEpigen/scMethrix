...
#' @param m Input \code{\link{scMethrix}} object
...
#' #' @export
#' #'
#' methrix_pca <- function(scm, var = "top", top_var = 1000, ranges = NULL,
...
#'     meth_sub <- subset_methrix(scm = scm, regions = ranges)
#'     meth_sub <- methrix::get_matrix(scm = meth_sub, type = "M", add_loci = FALSE)
#'   }
#'   
#'   if (is.null(top_var)) {
#'     message("All CpGs in the dataset will be used for the PCA")
#'     if (is.null(ranges)) {
#'       meth_sub <- get_matrix(scm = scm, type = "M", add_loci = FALSE)
#'     }
#'   } else {
...
#'                                                                                             nrow(scm)))))
#'       }
#'       meth_sub <- get_matrix(scm = scm[ids, ], type = "M", add_loci = FALSE)
#'     } else {
#'       if (!is.null(ranges)) {
#'         if (is_h5(scm)) {
...
#'                                                                        nrow(meth_sub)))], ]
#'       } else {
#'         meth_sub <- methrix::get_matrix(scm = order_by_sd(scm)[seq_len(min(top_var,
#'                                                                        nrow(scm)))], type = "M", add_loci = FALSE)
#'       }
#'     }
#'   }
#'   
...
#' @examples
#' data('scMethrix_data')
#' plot_coverage(m = scMethrix_data)
#' @export
...
#'   what <- match.arg(arg = what, choices = names(assays(scm)))
