test_that("read_index", {
  ...
  index <- read_index(files, col_idx = col_idx)
  expect_equal(names(index),c("chr","start","end"))
  expect_equal(dim(index),c(n_cpg,3))
  expect_equal(index,read_index(files,n_threads=2, col_idx = col_idx))
...
test_that("read_bed_by_index", {
  ...
  file <- files[1]
  index <- read_index(files, col_idx)
  C1 <- data.table::fread(file = file, sep = '\t', header = FALSE)
  bed <- read_bed_by_index(file,index,col_idx=col_idx)[[1]]
...
test_that("read_bed - threaded", {
  ...
  # expect_equal(dim(scm[[1]]),c(100,4))
  # expect_equal(scm[[1]],scm[[2]])
  # 
