...
# Changing genome assemblies
During sequencing, the reads will be aligned to some reference genome. For humans, this is typically either [hg19](https://www.ncbi.nlm.nih.gov/assembly/GCF_000001405.13/) or [hg38](https://www.ncbi.nlm.nih.gov/assembly/GCF_000001405.39). Experiments will different assemblies cannot be combined, and must be converted first. Via `[rtracklayer::liftOver](https://rdrr.io/bioc/rtracklayer/man/liftOver.html)`, `scMethrix` experiments can be easily converted.
...
There is an accessible `binarize` function to transform data into the beta ratio of [0..1] used by this package. The function will automatically generate a threshold cutoff based on the mean beta value. This is often useful after imputation.
...
scm.bin <- scMethrix::bin_scMethrix(scm, bin_size = 100000, bin_by = "bp")
```
...
scm.bin <- scMethrix::bin_scMethrix(scm, bin_size = 1000, bin_by = "cpg", trans = c(score= function(x) median(x,na.rm=TRUE)))
```
...
qhs = AnnotationHub::query(ah, c("RefSeq", "Mus musculus", "mm10"))
genes = qhs[[1]]
proms = promoters(genes)
proms = trim(proms) # Necessary as a few ranges throw warnings

scm.bin <- scMethrix::bin_scMethrix(scm, regions=proms)
```

To improve binning speed, it may be advisable to convert to an in-memory `scMethrix` objects. First, subset to the target region (this is done internally within `bin_scMethrix` anyways), convert to in-memory, then perform the binning.

```{r}
scm.bin <- scMethrix::subset_scMethrix(scm, regions = proms)
scm.bin <- scMethrix::convert_scMethrix(scm.bin, type = "memory")
scm.bin <- scMethrix::bin_scMethrix(scm, regions = proms)
```

# Collapse samples
It may be desirable to collapse multiple samples into a single meta-sample. Using simple grouping, samples can be collapsed via any arbitrary function. The `colname` used will become the new sample name, and the samples in each group will be listed in the `Samples` column of colData().

```{r}
# Collapse into simulated cluster_scMethrix() output
scm.collapse <- scMethrix::collapse_samples(scm.collapse, colname = "Cell")
colData(scm.collapse)
```

As well, the transformation for collapsing is user customizeable.

```{r}
# Collapse the samples while using a median operation for the score assay
...
scm.impute <- scMethrix::impute_regions(scm, assay = "score", new_assay = "impute", type = "RF")
head(score(scm))
```
...
scm.impute <- scMethrix::impute_regions(scm, assay = "score", new_assay = "impute", type = "kNN", regions = proms)
```
...
scm.impute <- scMethrix::impute_regions(scm, assay = "score", new_assay = "impute", type = fun)
```
