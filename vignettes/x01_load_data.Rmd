...
For this example, it can be easily extracted from the experiment's [SOFT file in GEO](https://ftp.ncbi.nlm.nih.gov/geo/series/GSE56nnn/GSE56879/soft/).
...
GSE_colData <- data.frame(Sample = names(GSE_colData), Cell = GSE_colData)
...
After the experiment is created, an assay matrix can be generated.  Additionally, the methylation and coverage matrices can be directly obtained with the functions `score()` and `counts()`.

```{r eval=F}
mtx <- get_matrix(scm, assay="score")
head(mtx)
```
Genomic loci can also be added (`add_loci`), be formatted as a `GenomicRanges` object if desired, and ordered by variance (`order_by_sd`).
